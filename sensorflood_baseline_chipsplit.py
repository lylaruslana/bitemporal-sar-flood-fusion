...
# ── CV split strategy ────────────────────────────────────────────────────────
# TILE  : original tile-level split (LEAKY — for comparison only)
# CHIP  : chip-level split (primary fix, recommended)
# EVENT : event-level split (strictest, geographic generalization)
CV_SPLIT_STRATEGY = "CHIP"
...
def train(model_id):
    ...
    print(f'Total tiles: {len(senforfloodmini)}')
    print(f'CV strategy: {CV_SPLIT_STRATEGY}')

    folds = make_folds(senforfloodmini, strategy=CV_SPLIT_STRATEGY, n_splits=5, seed=SEED)
    print_split_report(senforfloodmini, folds, strategy=CV_SPLIT_STRATEGY)
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        ...
        for epoch in range(max_epoch):
            ...
            # ── Train ─────────────────────────────────────────────────────
            model.train()
            for ind, (s1b, te, fm) in enumerate(train_loader):
                input_train = torch.cat([s1b[:, :3, :, :], te], dim=1).float().to(DEVICE)
            ...
            val_ious = []
            val_losses = []
            for s1b, te, fm in val_loader:
                input_eval = torch.cat([s1b[:, :3, :, :], te], dim=1).float().to(DEVICE)
    ...
    # ── Final Result ──────────────────────────────────────────────────────────
    print(f'\n{"="*40}')
    print(f'5-Fold CV Results ({CV_SPLIT_STRATEGY} split):')
...
if __name__ == '__main__':
    ...
    train(model_id=f'unet_efficientnetb3_Indonesia_baseline_{CV_SPLIT_STRATEGY.lower()}split')
