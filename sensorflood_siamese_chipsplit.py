...
class SiameseChangeNet(nn.Module):
    ...
    def __init__(self, encoder_name='efficientnet-b3', encoder_depth=5,
        ...
        bottleneck_ch = enc_ch[-1] + terrain_in
        modified_enc_ch = enc_ch[:-1] + [bottleneck_ch]
    ...
    def forward(self, s1_pre, s1_during, terrain):
        feat_pre = self.encoder(s1_pre)
        feat_dur = self.encoder(s1_during)
        ...
        diff[-1] = torch.cat([diff[-1], terrain_pooled], dim=1)
...
if __name__ == '__main__':
    ...
    train(model_id=f'unet_efficientnetb3_Indonesia_siamese_{CV_SPLIT_STRATEGY.lower()}split')
